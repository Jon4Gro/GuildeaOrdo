...
function UI.BuildMacros(f)
    ...
    f.macroMsg = CreateFrame("EditBox", "GuildeaOrdoMacroMsg", f.macroMsgBg)
    ...
    f.macroSaveBtn:SetScript("OnClick", function()
        ...
        if addon.editingMacroIndex then
            StaticPopupDialogs["GuildeaOrdo_CONFIRM_MACRO_OVERWRITE"] = { text = "Overwrite existing macro?", button1 = "Yes", button2 = "No", OnAccept = function() GuildeaOrdoDB.macros[addon.editingMacroIndex] = { channel = UI.macroSelectedChannel, text = text }; addon.editingMacroIndex = nil; f.macroMsg:SetText(""); f.macroMsg:ClearFocus(); UI:Refresh() end, timeout = 0, whileDead = true, hideOnEscape = true, preferredIndex = 3 }
            StaticPopup_Show("GuildeaOrdo_CONFIRM_MACRO_OVERWRITE")
        else
            local ok, msg = addon:AddMacro(UI.macroSelectedChannel, text)
            if ok then print("|cFFFFCC00GuildeaOrdo|r: macro saved."); f.macroMsg:SetText(""); f.macroMsg:ClearFocus() else print("|cFFFFCC00GuildeaOrdo|r: " .. tostring(msg)) end
            UI:Refresh()
        end
    end)

    f.macroSpamCB = CreateFrame("CheckButton", "GuildeaOrdoMacroSpamCB", f, "OptionsBaseCheckButtonTemplate")
    ...
    f.macroSpamCB:SetScript("OnClick", function(self)
        ...
        if GuildeaOrdoDB then GuildeaOrdoDB.spamTotalMinutes = addon.spamTotalLimit end
        ...
        if addon.spamMacros then for k, v in pairs(addon.spamMacros) do if v then hasActive = true; break end end end
            ...
            print("|cFFFFCC00GuildeaOrdo|r: Please 'Set' at least one macro first.")
            self:SetChecked(false)
            addon.spamActive = false
            addon.spamTotalElapsed = 0
                elseif addon.spamActive then
                print("|cFFFFCC00GuildeaOrdo|r: Macro Spam Started.")
                else
                print("|cFFFFCC00GuildeaOrdo|r: Macro Spam Ended.")
                end
                addon.spamTimer = 0
        end)
    ...
    f.macroSpamInterval = CreateFrame("EditBox", "GuildeaOrdoMacroSpamInterval", f, "InputBoxTemplate")
    ...
    f.macroSpamTotal = CreateFrame("EditBox", "GuildeaOrdoMacroSpamTotal", f, "InputBoxTemplate")
    ...
    f.macroSpamTotal:SetScript("OnTextChanged", function(self) addon.spamTotalLimit = tonumber(self:GetText()) or 0; if GuildeaOrdoDB then GuildeaOrdoDB.spamTotalMinutes = addon.spamTotalLimit end end)
    ...
    if GuildeaOrdoDB then
        local tval = GuildeaOrdoDB.spamTotalMinutes or 0
