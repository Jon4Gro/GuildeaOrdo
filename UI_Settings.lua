...
function UI.BuildSettings(f)
    ...
    f.batchSizeInput = CreateFrame("EditBox", "GildeaOrdoBatchSize", f, "InputBoxTemplate")
    ...
    f.batchSizeInput:SetScript("OnTextChanged", function(self) if GildeaOrdoDB then GildeaOrdoDB.batchSize = tonumber(self:GetText()) or 2 end end)

    f.openWithGuildCB = CreateFrame("CheckButton", "GildeaOrdoOpenWithGuildCB", f, "OptionsBaseCheckButtonTemplate")
    f.openWithGuildCB:SetSize(20, 20); f.openWithGuildCB:SetPoint("TOPLEFT", f.batchSizeLabel, "BOTTOMLEFT", 0, -12)
    f.openWithGuildCB:SetScript("OnClick", function(self) if GildeaOrdoCharDB then GildeaOrdoCharDB.openWithGuild = self:GetChecked() and true or false end end)
    f.openWithGuildLabel = f:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    f.openWithGuildLabel:SetPoint("LEFT", f.openWithGuildCB, "RIGHT", 4, 0); f.openWithGuildLabel:SetText("Open GildeaOrdo with Guild Frame")

    f.closeWithGuildCB = CreateFrame("CheckButton", "GildeaOrdoCloseWithGuildCB", f, "OptionsBaseCheckButtonTemplate")
    f.closeWithGuildCB:SetSize(20, 20); f.closeWithGuildCB:SetPoint("TOPLEFT", f.openWithGuildCB, "BOTTOMLEFT", 0, -6)
    f.closeWithGuildCB:SetScript("OnClick", function(self) if GildeaOrdoCharDB then GildeaOrdoCharDB.closeWithGuild = self:GetChecked() and true or false end end)
    f.closeWithGuildLabel = f:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    f.closeWithGuildLabel:SetPoint("LEFT", f.closeWithGuildCB, "RIGHT", 4, 0); f.closeWithGuildLabel:SetText("Close GildeaOrdo with Guild Frame")
    ...
    eb:SetScript("OnTextChanged", function(self) if GildeaOrdoDB then GildeaOrdoDB[dbKey] = self:GetText() end end)
    return eb
    end
    ...
    f.showMinimapCB = CreateFrame("CheckButton", "GildeaOrdoShowMinimapCB", f, "OptionsBaseCheckButtonTemplate")
    f.showMinimapCB:SetSize(20, 20); f.showMinimapCB:SetPoint("TOPRIGHT", -120, -90)
    f.showMinimapCB:SetScript("OnClick", function(self) if GildeaOrdoDB then GildeaOrdoDB.showMinimapButton = self:GetChecked() and true or false; if addon.UpdateMinimapButton then addon:UpdateMinimapButton() end end end)
    ...
    f.minimapDistSlider:SetScript("OnValueChanged", function(self, val) local v = math.floor(val / 2 + 0.5) * 2; v = math.max(20, math.min(240, v)); if GildeaOrdoDB then GildeaOrdoDB.minimapDistance = v end; if f.minimapDistVal then f.minimapDistVal:SetText(tostring(v)) end; if addon.UpdateMinimapPos then addon.UpdateMinimapPos() end end)
    ...
    f.autoInvCB = CreateFrame("CheckButton", "GildeaOrdoAutoInvCB", f, "OptionsBaseCheckButtonTemplate")
    f.autoInvCB:SetSize(24, 24); f.autoInvCB:SetPoint("TOPLEFT", f.autoInvLabel, "BOTTOMLEFT", 0, -20)
    f.autoInvCB:SetScript("OnClick", function(self) GildeaOrdoCharDB.autoInvite.enabled = self:GetChecked(); UI:Refresh() end)
    ...
    local function mkAIEB(name, labelTxt, width, anchorFrame, relPoint, x, y, dbKey)
        ...
        local eb = CreateFrame("EditBox", name, bg); eb:SetFontObject("ChatFontSmall"); eb:SetAutoFocus(false); eb:SetPoint("TOPLEFT", 6, -3); eb:SetPoint("BOTTOMRIGHT", -4, 3); eb:SetScript("OnEscapePressed", function(self) self:ClearFocus() end); eb:SetScript("OnTextChanged", function(self) if dbKey and GildeaOrdoDB and GildeaOrdoCharDB.autoInvite then GildeaOrdoCharDB.autoInvite[dbKey] = self:GetText() end end)
        return bg, eb, lbl
    end
    ...
    f.groupInviteCheck = CreateFrame("CheckButton", "GildeaOrdoGroupInviteCheck", f, "InterfaceOptionsCheckButtonTemplate"); f.groupInviteCheck:SetPoint("TOPLEFT", f.groupInviteLabel, "BOTTOMLEFT", 0, -13); GildeaOrdoGroupInviteCheckText:SetText("Auto Group On/Off: ")
    ...
    f.groupInviteEditBox = CreateFrame("EditBox", "GildeaOrdoGroupInviteEditBox", f, "InputBoxTemplate"); f.groupInviteEditBox:SetSize(490, 20); f.groupInviteEditBox:SetPoint("BOTTOMLEFT",  f.groupInviteCheck , "RIGHT", -18, -60); f.groupInviteEditBox:SetAutoFocus(false); f.groupInviteEditBox:SetScript("OnEscapePressed", function(self) self:ClearFocus() end); f.groupInviteEditBox:SetScript("OnEnterPressed", function(self) self:ClearFocus() end)
    f.groupInviteEditBox:SetScript("OnTextChanged", function(self) if GildeaOrdoDB and GildeaOrdoCharDB.autoInvite then GildeaOrdoCharDB.autoInvite.groupinv = self:GetText() or "" end end)

    f.groupInviteMinutes = CreateFrame("EditBox", "GildeaOrdoGroupInviteMinutes", f, "InputBoxTemplate"); f.groupInviteMinutes:SetSize(28, 20); f.groupInviteMinutes:SetPoint("LEFT", GildeaOrdoGroupInviteCheckText, "RIGHT", 8, 0); f.groupInviteMinutes:SetNumeric(true); f.groupInviteMinutes:SetAutoFocus(false); f.groupInviteMinutes:SetText("15")
    f.groupInviteMinutes:SetScript("OnTextChanged", function(self) if GildeaOrdoDB and GildeaOrdoCharDB.autoInvite then local v = tonumber(self:GetText()); GildeaOrdoCharDB.autoInvite.groupMinutes = (v ~= nil) and v or 15 end end); f.groupInviteMinutes:SetScript("OnEscapePressed", function(self) self:ClearFocus() end); f.groupInviteMinutes:SetScript("OnEnterPressed", function(self) self:ClearFocus() end)
    ...
    f:HookScript("OnShow", function()
        ...
        if GildeaOrdoDB and GildeaOrdoCharDB.autoInvite then
            if f.groupInviteEditBox then f.groupInviteEditBox:SetText(GildeaOrdoCharDB.autoInvite.groupinv or "") end
            if f.groupInviteMinutes then f.groupInviteMinutes:SetText(tostring(GildeaOrdoCharDB.autoInvite.groupMinutes or 15)) end
        end
    end)
    ...
    f.autoBanLeaveCB = CreateFrame("CheckButton", "GildeaOrdoAutoBanLeaveCB", f, "OptionsBaseCheckButtonTemplate")
    f.autoBanLeaveCB:SetSize(20, 20); f.autoBanLeaveCB:SetPoint("BOTTOMRIGHT", -18, 46)
    f.autoBanLeaveCB:SetScript("OnClick", function(self) if GildeaOrdoDB then GildeaOrdoDB.autoBanOnLeave = self:GetChecked() and true or false end end)
